...
[timholy/claude_config](https://github.com/timholy/claude_config) to
NonconvexUtils.jl.
...
- Preserve the type parameters of these `struct`s when constructing or passing
  ...
  — matrix-free vs. dense, symmetric vs. general — that dispatch on them. Do
  not silently strip them.
- `ImplicitFunction` differentiates through a fixed-point/iterative solve via
  the implicit function theorem. The `conditions` function must be zero at the
  solution returned by `forward`; the `tol` guard surfaces violations
  (fail-fast) instead of returning a silently wrong gradient. Keep this guard
  when modifying the rule.
- `LazyJacobian` stores only `jvp` and `jtvp` operators rather than a materialized
  matrix. Do not force materialization; operate through the operators so the
  laziness and the `matrixfree`/`symmetric` contracts are preserved.
- The package integrates with `NonconvexCore.flatten` / `tovecfunc` to flatten
  arbitrary (non-vector) inputs to vectors for AD backends that require them.
  When adding a wrapper, route its vector form through `tovecfunc` and preserve
  the unflattening of results and gradients.
- The `@ForwardDiff_frule` macro (`src/forwarddiff_frule.jl`) composes
  ForwardDiff `Dual`s with a ChainRulesCore `frule`. Reuse it rather than
  hand-writing Dual propagation when adding a new forward-mode-compatible
  wrapper.
